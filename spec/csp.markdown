...
NOTE: In order to describe the protocol concisely, some examples in this specification use HTTP/1.0, but in practice HTTP/1.1 is allowed and generally preferred.
    ...
    HTTP/1.0 POST http://example.org/csp/comet\r\n
    Content-length:4\r\n
    \r\n
    d={}
...
The SESSION_KEY for this CSP session is "abcdefg", as given by the handshake response. The format of the handshake response may be modified by the REQUEST_PREFIX and REQUEST_SUFFIX variables described in sections [3.3.3.11 REQUEST_PREFIX](#33311-request_prefix) and [3.3.3.12 REQUEST_SUFFIX](#33312-request_suffix). 
...
A PACKET_ENCODING with the value 1 means that the PACKET_DATA is [URL encoded](http://tools.ietf.org/html/rfc1738)  (also known as [percent encoded](http://en.wikipedia.org/wiki/Percent-encoding)). 
...
The DURATION variable signifies how long the server should leave a Comet request open before completing the response. A value of "0" will cause a response to always be sent immediately after a request is received, thus setting the connection mode to polling. Values of DURATION > 0 are used in conjuction with streaming and long polling, and will typically range from 10-45 seconds. A DURATION value of "0" will override IS_STREAMING with the value "0", forcing the connection into polling mode.
...
When a Comet HTTP request is received by the server, it will first process any variables given in the request. The varibles are encoded as a querystring, which can be given as the body of a POST or in the url of a GET, must contain the SESSION_KEY and ACK_ID. If the Last-Event-Id is present in the HTTP headers, its value will be used for the ACK_ID, and will supercede the value given in the querystring, if any. After parsing the querystring and headers, the server will adjust the persistent session variables and remove acknowleded packets from the buffer accordingly. 
...
and the body must follow the "Transfer-Encodng: chunked" format for HTTP/1.1.
...
The following string represents the packet batch that would be sent (beginning with c and ending with a semi-colon): 
...
The server should respond immediately with a 200 OK, with "OK" in the body of the response. When the client recieves an "OK", it can consider that any packets contained in the DATA variable to be acknowledged and may remove them from the buffer.
...
Even after the session is ended, a server is expected to continue to buffer any unacknowledged packets until a request is made that acknowledges the packets. The client, likewise, is expected to send a final request to acknowledge the last packet, which should be the [null packet](#3314-the-null-packet). The client should send its final acknowledge HTTP request to the /send url.
...
This is an example session of a server which acts as a CSP echo server. The client is using a script-tag long polling transport. Communication takes place over two TCP streams designated as TCP1 and TCP2.
    ...
    GET /csp/comet?s#abcdefg&bp#csp_packets&rs#; HTTP/1.1\r\n
    Pragma: no-cache\r\n
    Cache-Control: no-cache\r\n
    Hostname: www.example.com\r\n
    \r\n
    ...
    GET /csp/send?d#[1, Hello World]&s#abcdefg&rp#csp_sent_cb HTTP/1.1\r\n
    Pragma: no-cache\r\n
    Cache-Control: no-cache\r\n
    Hostname: www.example.com\r\n
    \r\n
...
This specification doesn't go into browser-specific implementation details of transports. Below, however, are examples of peristent connection variable values for some of the common transports.
